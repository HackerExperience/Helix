defmodule Helix.Server.Websocket.Channel.Server do
  ...
  @doc false
  def event_process_created(event = %ProcessCreatedEvent{}) do
    case event do
      %{gateway_id: gateway, target_id: gateway} ->
        notify(gateway, :processes_changed, %{})
      %{gateway_id: gateway, target_id: target} ->
        notify(gateway, :processes_changed, %{})
  ...
  @doc false
  def event_process_conclusion(event = %ProcessConclusionEvent{}) do
    notify(event.gateway_id, :processes_changed, %{})
    notify(event.target_id, :processes_changed, %{})
  end
end
