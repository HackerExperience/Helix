defmodule Helix.Process.Internal.TOP.Allocator.Plan do
  ...
  @spec plan([process], ServerResourcesTOP.t) ::
    {plan, ServerResourcesTOP.t}
    | {:error, any}
  defp plan(processes, resources) do
    plan = %{
      current_plan: %{fragment: %ServerResourcesTOP{}, processes: []},
  ...
  defp preallocate([h| t], plan, resources) do
    ...
    #   complicated
    case ServerResourcesTOP.sub_from_process(resources, process) do
      {:ok, resources} ->
  ...
  @spec allocable_resources(process, ServerResourcesTOP.t) ::
    [:cpu | :ram | :dlk | :ulk]
  ...
  defp execute_step(plan = %{current_plan: %{fragment: fragment, processes: [{process, required_resources}| t]}}, resources) do
    ...
    if allocated_before == allocated_after do
      ...
      resources = ServerResourcesTOP.sub_from_resources(
        resources,
        resources_diff,
        net_id)
  ...
  defp execute_step(plan = %{current_plan: %{processes: []}, next_plan: next_plan}, resources) do
    ...
    fragment = ServerResourcesTOP.part_from_shares(resources, shares)
