defmodule Helix.Test.Story.Setup do
  ...
  def story_step(opts \\ []) do
    ...
    related =
      related
      |> put_in([:manager], manager)
      |> put_in([:step], step)

    %{entry: inserted} = StepInternal.fetch_step(step.entity_id, step.contact)
    {inserted, related}
  end

  defp get_or_create_story_step(entity_id, contact_id) do
    with nil <- StoryQuery.fetch_step(entity_id, contact_id) do
      story_step(entity_id: entity_id, contact_id: contact_id)
    end
  end

  defp get_or_create_manager(entity_id) do
    with nil <- ManagerQuery.fetch(entity_id) do
      {manager, _} = StorySetup.Manager.manager(entity_id: entity_id)
  ...
  Opts:
  - entity_id - Fake entity ID is generated by default.
  - contact_id - Contact of the generated step used by default.
  - name - Must be used with `meta`. Defaults to random step from FakeStep
  - meta - Must be used with `name`. Defaults to random step from FakeStep
  ...
  def story_email(opts \\ []) do
    ...
    # Ensure the underlying (entity_id, contact_id) FK exists on the DB
    if not is_nil(opts[:entity_id]) and not is_nil(opts[:contact_id]) do
      get_or_create_story_step(opts[:entity_id], opts[:contact_id])
    end
  ...
  @doc """
  Related: Step.t, Entity.id, contact_id :: Step.contact, Step.email_id
  """
  def send_email(_opts \\ []) do
    {_, %{step: step, entity_id: entity_id}} = story_step()
