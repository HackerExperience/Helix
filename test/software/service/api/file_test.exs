defmodule Helix.Software.Service.API.FileTest do
  ...
  test "storage_contents/1 succeeds with valid input" do
    storage = Factory.insert(:storage)
    expected_files = Enum.count(storage.files)

    contents = API.storage_contents(storage)
    paths = Map.keys(contents)
    ...
    assert expected_files == Enum.count(files)
    assert Enum.all?(files, &(&1.path in paths))
    assert Enum.all?(files, &(&1 in files))
  end
