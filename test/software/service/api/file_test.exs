defmodule Helix.Software.Service.API.FileTest do
  ...
  test "storage_contents/1 succeeds with valid input" do
    storage = Factory.insert(:storage)
    refute Enum.empty?(API.storage_contents(storage))
  end
  ...
  describe "copy/3" do
    test "succeeds with valid input" do
      ...
      new_storage = Factory.insert(:file)
    ...
    test "fails when input is invalid" do
      origin = Factory.insert(:file)
      new_storage = Factory.build(:storage)
      new_path = ""

      result = API.copy(origin, origin.storage, new_path)
      assert {:error, %Ecto.Changeset{}} = result

      result = API.copy(origin, new_storage, origin.path)
      assert {:error, %Ecto.Changeset{}} = result

      result = API.copy(origin, origin.storage, origin.path)
      assert {:error, %Ecto.Changeset{}} = result
    end
  end
  ...
  describe "move/2" do
    ...
    test "fails when input is invalid" do
      conflict = Factory.insert(:file)
      similarities = Map.take(conflict, [:name, :software_type, :storage])
      file = Factory.insert(:file, similarities)

      assert {:error, %Ecto.Changeset{}} = API.move(file, conflict.path)
    end
  end
  ...
  describe "rename/2" do
    ...
    test "fails when input is invalid" do
      conflict = Factory.insert(:file)
      similarities = Map.take(conflict, [:path, :software_type, :storage])
      file = Factory.insert(:file, similarities)

      assert {:error, %Ecto.Changeset{}} = API.rename(file, conflict.name)
    end
  end
