defmodule Helix.Core.Listener.Event.Handler.ListenerTest do
  ...
  describe "Event handling" do
    test "objects being listened will have their callback called" do
      file_downloaded_event = EventSetup.Software.file_downloaded()

      listen file_downloaded_event.file.file_id, FileDownloadedEvent, :callback,
        owner_id: file_downloaded_event.entity_id,
        subscriber: :teste_daora

      Event.emit(file_downloaded_event)
    ...
    test "won't receive anything if event did not match" do
      ...
      listen file_downloaded_event.file.file_id, FileUploadedEvent, :callback,
        owner_id: file_downloaded_event.entity_id,
        subscriber: :teste_daora
    ...
    test "multiple entries get called multiple times; meta works too" do
      ...
      listen file_downloaded_event.file.file_id, FileDownloadedEvent, :callmeta,
        meta: %{entity_id: file_downloaded_event.entity_id},
        owner_id: file_downloaded_event.entity_id,
      ...
      assert meta["entity_id"] == to_string(file_downloaded_event.entity_id)
    end
  end
end
