defmodule Helix.Universe.Bank.Websocket.Channel.Bank.JoinTest do
  ...
  describe "BankJoin" do
    test "can join a bank account with password" do
    ...
    test "can join a bank account with token" do
      {socket, %{account: player, server: gateway}} =
        ChannelSetup.create_socket()
    ...
    test "can't join bank account with expired token" do
      ...
      bank_account = BankSetup.account!()
      token = BankSetup.token!(acc: bank_account, expired: true)
      token_id = token.token_id
      ...
      assert reason.data == "token_expired"
    end

    test "can't join bank account with not matching token" do
      {socket, %{server: gateway}} =
        ChannelSetup.create_socket()
      ...
      bank_account = BankSetup.account!()
      token = BankSetup.token!(expired: true)
      token_id = token.token_id
      ...
      assert reason.data == "token_not_belongs"
    end
